...
function getJobInfo(sysCd) {
	...
	ajaxAsync('/webPage/program/ProgramInfoServlet', tmpInfoData, 'json', successJob);
}
...
function successJob(data) {
	...
	if (cboJobData != null && (cboJobData.length > 0)) {
		$('[data-ax5select="cboJob"]').ax5select({
	        options: injectCboDataToArr(cboJobData, 'cm_jobcd', 'jobname')
	   	});
		...
		cboAftJobData.unshift({cm_jobcd: '0', jobname: '선택하세요'});
		$('[data-ax5select="cboAftJob"]').ax5select({
	        options: injectCboDataToArr(cboAftJobData, 'cm_jobcd', 'jobname')
	   	});
...
function btnQry_Click() {
	...
	ajaxAsync('/webPage/administrator/AllUpdateServlet', tmpInfoData, 'json', successPrjList);
}
