...
function successSysInfo(data) {
	...
	lstSysData = lstSysData.filter(function(data) {
		if(data.cm_syscd == "00000") return false;
		else return true;
	});
...
function btnQry_Click() {
	...
	tmpInfo = new Object();
	tmpInfo.SysCd = getSelectedVal('cboFromSys').value;
	tmpInfo.ReqCd = getSelectedVal('cboFromReqCd').value;
...
function btnCopy_Click() {
	...
	if($('#chkAllCopy').is(':checked')) {
		...
		tmpInfoData = {
			tmpInfo		: tmpInfo,
			tmpArray	: checkedGridItem,
			requestType	: 'COPYAPPROVALINFO'
		}
		ajaxAsync('/webPage/administrator/ApprovalInfoServlet', tmpInfoData, 'json', successCopyApprovalInfo);
	}
}
