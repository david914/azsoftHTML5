...
function getProgInfo() {
	...
	ajaxAsync('/webPage/winpop/PopScriptServlet', data, 'json',successGetProgInfo);
}
//최초 스크립트 데이타 조회완료
function successGetProgInfo(data) {
	...
	if (baseData.length > 0) {
		...
		ajaxAsync('/webPage/winpop/PopScriptServlet', data, 'json',successGetBldList);
	}
}
...
function successGetBldList(data){
	...
	$.each(cboScriptData,function(key,value) {
	    options.push({ value: value.cm_micode, text: value.cm_codename, 
	    	 		   ...
	    	 		   cm_execyn:value.cm_execyn,  cm_bldcd:value.cm_bldcd,  cm_prcsys:value.cm_prcsys });
	});
...
function getBldScript() {
	...
	data = {
    	...
    	BldGbn		: getSelectedVal('cboScript').value,
    	BldCd		: getSelectedVal('cboScript').cm_bldcd,
    	ItemId		: pItemId,
    	PrcSys		: getSelectedVal('cboScript').cm_prcsys,
		requestType : 'getBldScript'
	}
	ajaxAsync('/webPage/winpop/PopScriptServlet', data, 'json',successGetBldScript);
}
