...
function createViewGrid() {
	...
	if (jobGridData != null && jobGridData.length > 0) {
...
function screenLoad() {
	...
	$('#cboSys').bind('change',function(){
		if (getSelectedVal('cboSys').value == '00000') $('#txtSysCd').val('');
		else $('#txtSysCd').val(getSelectedVal('cboSys').value);
	});
	
	// 시스템 속성 클릭시
	$('input.checkbox-sysInfo').bind('click', function(e) {
		
		if ((!$('#chkOpen').is(':checked') && selectedSystem == null) ||
		   ($('#chkOpen').is(':checked') && $('txtSysCd').val().length != 5)) {
...
function setSysInfo() {
	...
	if(selectedSystem.cm_systype !== undefined) {
	...
	getSysJobInfo(selectedSystem.cm_syscd);
...
function sysValidationCheck() {
	...
	if( $('#chkOpen').is(':checked')) {
		...
		if ($('#txtSysCd').val() != getSelectedVal('cboSys').value) {
			dialog.alert('입력한 시스템코드와 선택된 시스템코드가 불일치합니다.',function(){});
			return;
		}
	}
...
function successGetSysInfoList(data) {
	...
	var sysInfoData = data;
	if (sysInfoData.length != 1) {
		dialog.alert('등록된 시스템정보 가져오기에 실패하였습니다.');
		return;
	}
	window.parent.selectedSystem = sysInfoData[0];
...
function getUseSysCdList() {
	
	var useSysListCboData;
	
	useSysListCboData = new Object();
	useSysListCboData = {
		requestType	: 	'getUseSysCdList'
	}
	ajaxAsync('/webPage/administrator/SysInfoServlet', useSysListCboData, 'json',successGetUseSysList);
	
}

function successGetUseSysList(data) {
...
function sysDataFilter(){
	...
	var cnt = 0;
	cboOptions.push({value: '00000', text: '선택하세요'});
	var inSysCd = $('#txtSysCd').val();
	for(var i=0; i<cboSysCdData.length ; i++){
		var data = cboSysCdData[i];
		if (inSysCd.length > 2) {
			if (data.cm_syscd.substr(0,3) == inSysCd.substr(0,3)) {
				cboOptions.push({value: data.cm_syscd, text: data.cm_sysmsg});
				if (++cnt > 300) break;
			}
		} else {
			cboOptions.push({value: data.cm_syscd, text: data.cm_sysmsg});
