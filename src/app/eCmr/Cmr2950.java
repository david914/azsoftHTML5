...
//		Object[]          returnObjectArray    = null;
		...
		try {
			...
			ecamsLogger.error("## Cmr2950.getReqList() SQLException START ##");
			ecamsLogger.error("## Error DESC : ", sqlexception);
			ecamsLogger.error("## Cmr2950.getReqList() SQLException END ##");
			throw sqlexception;
		} catch (Exception exception) {
			exception.printStackTrace();
			ecamsLogger.error("## Cmr2950.getReqList() Exception START ##");
			ecamsLogger.error("## Error DESC : ", exception);
			ecamsLogger.error("## Cmr2950.getReqList() Exception END ##");
			throw exception;
		}finally{
			...
			if (conn != null){
				try{
					...
					ecamsLogger.error("## Cmr2950.getReqList() connection release exception ##");
					ex3.printStackTrace();
				}
			}
		}
	...
	public Object[] getProgList(String AcptNo) throws SQLException, Exception {
		...
		try {
			...
			conn = connectionContext.getConnection();
			strQuery.setLength(0);
			strQuery.append(" select a.cd_gbnname,a.cd_detname,a.cd_subname, a.cd_rstdefault, b.*\n");
			strQuery.append("   from CMD1900 a, cmr2900 b                                        \n");
			...
			strQuery.append("    and a.cd_subname=b.cr_subname                                   \n");
			strQuery.append("  order by a.cd_gbncd,a.cd_detcd, a.cd_suborder                     \n");
	        pstmt = conn.prepareStatement(strQuery.toString());
			//pstmt = new LoggableStatement(conn,strQuery.toString());
			pstmt.setString(1, AcptNo);
	        //ecamsLogger.error(((LoggableStatement)pstmt).getQueryString());
	        ...
	        while (rs.next())
	        {
	        	...
	        	if(rs.getString("cd_detname").equals(detname) || rs.getString("cd_detname")==detname){
	        		rst.put("cd_detname", "");
	        	}else{
	        	...
	        	rst.put("cd_rstdefault", rs.getString("cd_rstdefault"));

	        	rst.put("cr_rststa", rs.getString("CR_RSTSTA"));
			...
			sqlexception.printStackTrace();
			ecamsLogger.error("## Cmr2950.getProgList() SQLException START ##");
			ecamsLogger.error("## Error DESC : ", sqlexception);
			ecamsLogger.error("## Cmr2950.getProgList() SQLException END ##");
			throw sqlexception;
		} catch (Exception exception) {
			exception.printStackTrace();
			ecamsLogger.error("## Cmr2950.getProgList() Exception START ##");
			ecamsLogger.error("## Error DESC : ", exception);
			ecamsLogger.error("## Cmr2950.getProgList() Exception END ##");
			throw exception;
		}finally{
			if (strQuery != null) 	strQuery = null;
			if (rsval != null) rsval = null;
			...
			if (conn != null){
				try{
					...
					ecamsLogger.error("## Cmr2950.getProgList() connection release exception ##");
					ex3.printStackTrace();
				}
			}
		}
	...
	public ArrayList<HashMap<String, String>> confLocat_conn(String AcptNo,String PrcSw,String CnclStep,
		...
		try {
			...
			if (rs != null)     try{rs.close();}catch (Exception ex){ex.printStackTrace();}
			if (pstmt != null)  try{pstmt.close();}catch (Exception ex2){ex2.printStackTrace();}
			ecamsLogger.error("## Cmr2950.confLocat_conn() SQLException START ##");
			ecamsLogger.error("## Error DESC : ", sqlexception);
			ecamsLogger.error("## Cmr2950.confLocat_conn() SQLException END ##");
			throw sqlexception;
		} catch (Exception exception) {
			exception.printStackTrace();
			if (rs != null)     try{rs.close();}catch (Exception ex){ex.printStackTrace();}
			if (pstmt != null)  try{pstmt.close();}catch (Exception ex2){ex2.printStackTrace();}
			ecamsLogger.error("## Cmr2950.confLocat_conn() Exception START ##");
			ecamsLogger.error("## Error DESC : ", exception);
			ecamsLogger.error("## Cmr2950.confLocat_conn() Exception END ##");
			throw exception;
		}finally{
			if (strQuery != null) 	strQuery = null;
			if (rsval != null)  rsval = null;
